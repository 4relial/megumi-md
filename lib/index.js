...
exports.core = async (sock, mei) => {
    ...
    try {
        ...
        if (mei) {
            ...
            const isPrem = premiumNumber.includes(sender)
            ...
            var g = {
                ...
                func: {
                  ...
                  async konachan(charname, length, next, name) {
                        zc.konachan(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                          if(img == "error"){
                             console.log("(Image Not Found) : Retry Now!")
                             return g.func.konachan(charname, length, next, name)
                          }
         
                            if (img[0].image) {
                              if(img[0].rating != "s"){
                             console.log("(isNSFW!) : Retry Now!")
                             return g.func.konachan(charname, length, next, name)
                          }
                              const link = img[0].image
                              const source = img[0].source
