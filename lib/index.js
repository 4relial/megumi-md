...
const { MessageRetryHandler } = require('./store')
...
exports.core = async (sock, mei) => {
    ...
    try {
        ...
        if (mei) {
            ...
            const body = global.d.body(type, mei)
            ...
            var g = {
                ...
                func: {
                    async reply(txt) {
                        ...
                        var response = await sock.sendMessage(from, { text: txt }, { quoted: mei }).then(handler.addMessage);
                        return response
                    },
                    parameter(body) {
                    ...
                    async report(txt = "ada laporan") {
                        await sock.sendReadReceipt(from, participant, [id])
                        var response = await sock.sendMessage("6289515275674@s.whatsapp.net", { text: txt }, { quoted: mei }).then(handler.addMessage);
                        return response
                    },
                    async notPremium(txt) {
                        await sock.sendReadReceipt(from, participant, [id])
                        var response = await sock.sendMessage(from, { text: "Fitur khusus user premium!" }, { quoted: mei }).then(handler.addMessage);
                        return response
                    },
                    ...
                    async replySticker(path) {
                        await sock.sendReadReceipt(from, participant, [id])
                        await sock.sendMessage(from, { sticker: { url: path } }, { quoted: mei }).then(handler.addMessage);
                    },
                    async imagetosticker(path, teks) {
                        ...
                        await sock.sendMessage(from, await sticker.toMessage(), { quoted: mei }).then(handler.addMessage);
                        fs.unlinkSync(path)
                    },
                    ...
                    async sh(charname, length, next) {
                        let response
                        zc.getSearchSS(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                            const num = Math.floor(Math.random() * 14) + 1;


                            if (img[num]?.image) {
                                console.log(img[num]?.image)
                                ...
                                const template = generateWAMessageFromContent(from, proto.Message.fromObject({
                                    templateMessage: {
                                        hydratedTemplate: {
                                            imageMessage: message.imageMessage,
                                            hydratedContentText: "Source: https://e-shuushuu.net",
                                            hydratedFooterText: 'Megumi ~Bot',
                                            hydratedButtons: [{
                                                quickReplyButton: {
                                                    displayText: 'Next➠',
                                ...
                                console.log("(page not found) : Retry Now!")
                                return g.func.shuushuu(charname, length, next)
                            }
                        });
                    },
                    async shTop(length, next) {
                        let response
                        zc.getSearchSSR(Math.floor(Math.random() * length) + 1).then(async (img) => {
                            const num = Math.floor(Math.random() * 14) + 1;
                    ...
                    async danbooru(charname, length, next) {
                        zc.danbooru(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                            if (img == "error") {
                                console.log("(Image Not Found) : Retry Now!")
                                return g.func.danbooru(charname, length, next)
                            }

                            if (img[0].image) {
                                const link = img[0].image
                                const source = img[0].source
                                const tag = img[0].cr
                                console.log(link)
                                ...
                                sock.relayMessage(from, template.message, { messageId: template.key.id }).then(handler.addMessage);
                    ...
                    async konachan(charname, length, next, name, nsfw) {
                        zc.konachan(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                            if (img == "error") {
                                console.log("(Image Not Found) : Retry Now!")
                                return g.func.konachan(charname, length, next, name, nsfw)
                            }

                            if (img[0].image) {
                                if (img[0].rating != "s" && !isNotFilterx) {
                                    console.log("(isNSFW!) : "+img[0].image)
                                    return g.func.konachan(charname, length, next, name, nsfw)
                                }
                                const link = img[0].image
                                const source = img[0].source
                    ...
                    async replyImage(path, description) {
                        await sock.sendReadReceipt(from, participant, [id])
                        await sock.sendMessage(from, { image: { url: path }, caption: description }, { quoted: mei }).then(handler.addMessage);
                    },
                    async send(to, txt) {
                        await sock.sendMessage(to, { text: txt }).then(handler.addMessage);
                    },
                    async replyMarked(txt, members) {
                        await sock.sendMessage(from, { text: txt, contextInfo: { mentionedJid: members } }, { quoted: mei }).then(handler.addMessage);
                    },
                    ...
                    case `ytmp3katou`:
                        {
                            ...
                            try {
                                let path = `./datamegumi/${g.func.parameter(body)}.mp3`
                                ...
                                YD.download(g.func.parameter(body), `${g.func.parameter(body)}.mp3`);
                    ...
                    case `del`:
                        {
                            ...
                            if (group && !owner && !JSON.stringify(groupAdmins).includes(sender)) return
 ...
 //FilterX
                    case 'nsfw-on': {
                        ...
                        try {
                            ...
                            const cekk = filterx.includes(from);
                            if (cekk) {
                                g.func.reply('NSFW Telah Aktif!')
                            } else {
                                await filterx.push(from)
                                fs.writeFileSync('lib/modules/nsfwfilter.json', JSON.stringify(filterx))
                                g.func.reply('NSFW Diaktifkan')
                    ...
                    case 'nsfw-off': {
                        ...
                        try {
                            ...
                            const cekk = filterx.includes(from);
                            if (cekk) {
                                const indexban = await filterx.indexOf(from)
                                await filterx.splice(indexban, 1)
                                fs.writeFileSync('lib/modules/nsfwfilter.json', JSON.stringify(filterx))
                                g.func.reply('NSFW DI Non-Aktifkan!')

                            } else {

                                g.func.reply('NSFW Telah Nonaktif')
                            }
                    ...
                    case `premium`:
                     ...
                     case 'tst': {
                       sock.sendMessage("6281282378763-1601122034@g.us", { text: "Hello there!" }).then(handler.addMessage);
                     }
                }
            }
