    ...
    try {
        ...
        var g = {
            ...
            func: {
              ...
              async zerochan2(charname, length, next) {
                    zc.getSearch2(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                        const num = Math.floor(Math.random() * 23) + 1;
                        ...
                        if (img[num]?.image) {
                            ...
                            console.log ("retry")
                            return g.func.zerochan2(charname, length, next)
                        }
                    });
                },
...
function parseInput2(keyword) {
	const query = keyword.split('(');
	return query.join('%28');
}
function parseInput3(keyword) {
	const query = keyword.split(')');
	return query.join('%29');
}
function parseInput4(keyword) {
	const query = keyword.split('!');
	return query.join('%21');
}
                        if (img[num]?.image) {
                            console.log(img[num]?.nama);
                            console.log(img[num]?.image);
                            var name0 = parseInput(img[num]?.nama)
                            var name1 = parseInput2(name0)
                            var name2 = parseInput3(name1)
                            var name3 = parseInput4(name2)
                            var n1 = img[num].image;
                            var n2 = n1.replace('/', '');
                            let link = "https://static.zerochan.net/"+name3+".full."+n2+".jpg"
                            console.log(link)
                            try{
                            let message = await prepareWAMessageMedia({ image: {url: link} }, { upload: sock.waUploadToServer })
                            const template = generateWAMessageFromContent(from, proto.Message.fromObject({
                                templateMessage: {
                                    hydratedTemplate: {
                                        imageMessage: message.imageMessage,
                                        hydratedContentText: img[num]?.nama,
                                        hydratedFooterText: 'Megumi ~Bot',
                                        hydratedButtons: [{
                                            quickReplyButton: {
                                                displayText: 'Next➠',
...
Anime: ${anim}`;
                ...
                case `randomimage`:
                    {
                        const teks = daftarTeks.menuGame()
