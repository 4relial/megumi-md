...
exports.global = {
    ...
    d: {
        verificarMei(mei) {
            ...
            // if (mei.key.id.startsWith('BAE5') && mei.key.id.length === 16) return false
            if (mei.key.fromMe) return false
            return mei
        },
        body(type, msg) {
            return JSON.parse(JSON.stringify((type === 'conversation' && msg.message.conversation)
                ? msg.message.conversation
                : (type == 'templateButtonReplyMessage') && msg.message.templateButtonReplyMessage
                    ? msg.message.templateButtonReplyMessage.selectedId
                    : (type == 'buttonsResponseMessage') && msg.message.buttonsResponseMessage
                        ? msg.message.buttonsResponseMessage.selectedButtonId
                        : (type == 'listResponseMessage') && msg.message.listResponseMessage
