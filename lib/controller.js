...
*Description:* ${character.description}`;
		...
		const z = g.is.body.toLowerCase().indexOf('kontol')
		const z1 = g.is.body.toLowerCase().indexOf(' asu')
		const z2 = g.is.body.toLowerCase().indexOf('asu ')
		const x5 = g.is.body.toLowerCase().indexOf(' bot')
		const z3 = g.is.body.toLowerCase().indexOf('bgsd')
		const x1 = g.is.body.toLowerCase().indexOf('bot ')
		const x2 = g.is.body.toLowerCase().indexOf('megumi')
		const x3 = g.is.body.toLowerCase().indexOf('ohayo')
		const x4 = g.is.body.toLowerCase().indexOf('selamat pagi')

		if (z !== -1 || z1 !== -1 || z2 !== -1 || z3 !== -1) {
			x.replySticker('./Angry!.webp')
		} else if (x1 !== -1 || x2 !== -1 || x3 !== -1 || x4 !== -1 || x5 !== -1) {
			x.replySticker('./^_^.webp')
		}
		if (bd == 'asu' || bd == 'bot') {
			x.replySticker('./huh.webp')
		}
	}
}