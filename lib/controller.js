...
*Description:* ${character.description}`;
          ...
          const name = capt.toLowerCase()
			...
			case "play":
				{
          if (!g.is.dono && !g.is.isPrem) return x.notPremium()
         ...
         .then(waifu => {
           if(waifu.code == 200 || waifu.code == 201){
             ...
             await g.func.zerochan3("user/Doopliss", 300, body)
           }
					break
				}
...
case "uniform":
        ...
        case `fanart`:
          ...
          await g.func.zerochan3("user/victus007", 99, body)
					break;
				}
        case `lily`:
				{
        ...
        case `kemonomimi`:
				{
          if (!g.is.dono && !g.is.isPrem) return x.notPremium()
					g.func.zerochan2("Kemonomimi,female?s=fav&t=0", 99, body)
					break;
				}
        case `neko`:
				{
					g.func.zerochan2("nekomimi,female?s=fav&t=0", 99, body)
					break;
				}
			case `azurlane`:
				{
		...
		if (!g.is.isbanned && z !== -1 || z1 !== -1 || z2 !== -1 || z3 !== -1 || z4 !== -1) {
			x.replySticker('./Angry!.webp')
		} else if (!g.is.isbanned && x1 !== -1 || x2 !== -1 || x3 !== -1 || x4 !== -1 || x5 !== -1) {
