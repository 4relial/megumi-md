...
const { default: WASocket, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion } = require("@adiwajshing/baileys")
...
exports.createConnection = new Promise((resolve, reject) => {
    ...
    const sock = WASocket({ version, printQRInTerminal: true, auth: state,logger: Pino({ level: "silent" }) })
try{
sock.ev.on('messages.upsert', async m => await core(sock, m))
  ...
  console.log(e)
        ...
        if (connection === 'close') lastDisconnect.error?.output?.statusCode !== DisconnectReason.loggedOut
            ? startSock() : fs.unlinkSync("./.megumisecret/WAConnection.json")
    })
